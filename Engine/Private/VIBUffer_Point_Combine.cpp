#include "..\Public\VIBuffer_Point_Combine.h"

CVIBuffer_Point_Combine::CVIBuffer_Point_Combine(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CVIBuffer(pDevice, pDeviceContext)
{
}

CVIBuffer_Point_Combine::CVIBuffer_Point_Combine(const CVIBuffer_Point_Combine & rhs)
	: CVIBuffer(rhs)
...
HRESULT CVIBuffer_Point_Combine::NativeConstruct_Prototype(_uint iNumInstance)
{
...
#pragma region INSTANCE_BUFFER
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		_float4 RandPos = _float4(_float(rand() % 11 - 5), _float(rand() % 11 - 5), _float(rand() % 11 - 5), 1.f);
		_float4 fDir = _float4(-RandPos.x, -RandPos.y, -RandPos.z, 0.f);
		...
		pInstanceVertices[i].vTranslation = RandPos;
		pInstanceVertices[i].vDirection = fDir;
	}
...
HRESULT CVIBuffer_Point_Combine::NativeConstruct(void * pArg)
{
	return S_OK;
}

HRESULT CVIBuffer_Point_Combine::Update(_double TimeDelta)
{
	...
	for (_uint i = 0; i < m_iNumInstance; ++i)
	{
		...
		if (XMVectorGetX(XMVector3Length(vPosition)) < 0.5f) {
			vPosition = XMVectorSet(0.f, 0.f, 0.f, 0.f);
			m_pInstanceSpeed[i] = 0;
		}
		else
			flag = true;
...
HRESULT CVIBuffer_Point_Combine::Render()
{
	if (nullptr == m_pDeviceContext)
		return E_FAIL;
...
CVIBuffer_Point_Combine * CVIBuffer_Point_Combine::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, _uint iNumInstance)
{
	CVIBuffer_Point_Combine*	pInstance = new CVIBuffer_Point_Combine(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype(iNumInstance)))
	{
		MSG_BOX(TEXT("Failed to Created CVIBuffer_Point_Combine"));
...
CComponent * CVIBuffer_Point_Combine::Clone(void * pArg)
{
	CVIBuffer_Point_Combine*	pInstance = new CVIBuffer_Point_Combine(*this);
...
void CVIBuffer_Point_Combine::Free()
{
	__super::Free();
