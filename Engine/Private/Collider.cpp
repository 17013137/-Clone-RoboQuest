...
HRESULT CCollider::NativeConstruct(void * pArg)
{
	...
	{
		m_ColliderDesc.vPosition = _float3(0.f, 0.f, 0.f);
		m_ColliderDesc.vSize = _float3(1.f, 1.f, 1.f);
		m_ColliderDesc.fRadius = 1.f;
	}	

	switch (m_eType)
	{
	case CCollider::TYPE_AABB:
		m_pAABB = new BoundingBox(m_ColliderDesc.vPosition, _float3(m_ColliderDesc.vSize.x * 0.5f, m_ColliderDesc.vSize.y * 0.5f, m_ColliderDesc.vSize.z * 0.5f));
		m_pAABBInWorld = new BoundingBox();
		break;
	case CCollider::TYPE_OBB:
		m_pOBB = new BoundingOrientedBox(m_ColliderDesc.vPosition, _float3(m_ColliderDesc.vSize.x * 0.5f, m_ColliderDesc.vSize.y * 0.5f, m_ColliderDesc.vSize.z * 0.5f), _float4(0.f, 0.f, 0.f, 1.f));
		m_pOBBInWorld = new BoundingOrientedBox();
		break;
	case CCollider::TYPE_SPHERE:
		m_pSPHERE = new BoundingSphere(m_ColliderDesc.vPosition, m_ColliderDesc.fRadius);
...
_bool CCollider::Collision_Point(_fvector vPos1, _fvector vPos2, _fvector vPos3)
{
	_bool flag = false;
	...
	case Engine::CCollider::TYPE_AABB:
		flag = m_pAABBInWorld->Intersects(vPos1, vPos2, vPos3);
		break;
	case Engine::CCollider::TYPE_OBB:
		flag = m_pOBBInWorld->Intersects(vPos1, vPos2, vPos3);
		break;
	case Engine::CCollider::TYPE_SPHERE:
		flag = m_pSPHEREInWorld->Intersects(vPos1, vPos2, vPos3);
		break;
	default:
		break;
	}
...
_vector CCollider::InterSectsRay(_float3 RayPos, _float3 RayDir)
{
	_float Dist = -1.0f;
	_bool flag = false;

	_float3 fRayPos = RayPos;
	_vector vRay =  XMLoadFloat3(&RayDir);
	_vector vRayPos = XMLoadFloat3(&RayPos);
	
	switch (m_eType)
	{
	case Engine::CCollider::TYPE_AABB:
		flag = m_pAABBInWorld->Intersects(vRayPos, vRay, Dist);
		break;
	case Engine::CCollider::TYPE_OBB:
		flag = m_pOBBInWorld->Intersects(vRayPos, vRay, Dist);
		break;
	case Engine::CCollider::TYPE_SPHERE:
		flag = m_pSPHEREInWorld->Intersects(vRayPos, vRay, Dist);
		break;
	default:
		break;
	}
	if (flag == true) {
		vRay = XMVector3Normalize(vRay) * Dist;
		vRayPos += vRay;
		return vRayPos;
	}
...
_bool CCollider::InterSectSphere(const BoundingSphere& Src, _float* Dist)
{
	_bool flag = false;
	...
	{
	case Engine::CCollider::TYPE_AABB:
		flag = m_pAABBInWorld->Intersects(Src);
		break;
	case Engine::CCollider::TYPE_OBB:
		flag = m_pOBBInWorld->Intersects(Src);
		break;
	case Engine::CCollider::TYPE_SPHERE:
		flag = m_pSPHEREInWorld->Intersects(Src);
		if(Dist != nullptr)
			*Dist = fabsf((Src.Radius + m_pSPHEREInWorld->Radius) - (XMVectorGetX(XMVector3Length(XMLoadFloat3(&Src.Center) - XMLoadFloat3(&m_pSPHEREInWorld->Center)))));
		break;
	default:
		break;
	}
...
_bool CCollider::InterSectOBB(const BoundingOrientedBox & Src, _float * Dist)
{
	_bool flag = false;
...
_bool CCollider::InterSectAABB(const BoundingBox & Src, _float * Dist)
{
	_bool flag = false;
...
void CCollider::Update(_fmatrix WorldMatrix)
{
	...
	m_ColliderDesc.vWorldPosition = _float3(fWorldMatrix.m[3][0] + m_ColliderDesc.vPosition.x, fWorldMatrix.m[3][1] + m_ColliderDesc.vPosition.y, fWorldMatrix.m[3][2] + m_ColliderDesc.vPosition.z);
}

#ifdef _DEBUG
HRESULT CCollider::Render()
{
	_vector		vColor = m_isCollision == true ? XMVectorSet(1.f, 0.f, 0.f, 1.f) : m_LineColor;
